...
def add_complements(marker_exp):
    ...
    a given gene X are multiplied by -1 and become a new column, labeled
    X_c. "High" expression values of X become "low" values of X_c, and vice
    versa, where discrete expression corresponds to a "high" value, and
    discrete non-expression to a "low" value.

    :param marker_exp: A DataFrame whose rows are cell identifiers, columns are
    gene identifiers, and values are float values representing gene expression.
    :returns: A DataFrame of same format as marker_exp, but with a new column
    added for each existing column label, representing the column label gene's
    complement.
...
def batch_xlmhg(marker_exp, c_list, coi, X=None, L=None):
    ...
    are the stat, cutoff, and pval outputs of the XL-mHG test; of float, int,
    and float type respectively. Their names are 'HG_stat', 'mHG_cutoff', and
    'mHG_pval'.
    :rtype: pandas.DataFrame
...
def batch_t(marker_exp, c_list, coi):
    ...
    :param coi: The cluster of interest.
    :returns: A matrix whose row labels are gene identifiers, and whose columns
    are the t statistic and t p-value; both of float type. Their names are
    't_stat' and 't_pval'.
    :rtype: pandas.DataFrame
...
def mhg_slide(marker_exp, cutoff_val, mhg_output):
    ...
    uniform expression inside the sorted expression list. I.e. for a population
    of cells of which many have zero expression, the XL-mHG test may demand
    that we sample some of the zero-expression cells and not others. This is
    impossible because the cells are effectively identical. This function
    therefore moves the XL-mHG cutoff index so that it falls on a measurable
    ...
    whose columns are the stat, cutoff, and pval XL-mHG outputs for that gene.
...
def TP_TN(discrete_exp, c_list, coi):
    ...
    containing the true positive and true negative values respectively. Their
    names are 'TP' and 'TN'.
    :rtype: pandas.DataFrame
